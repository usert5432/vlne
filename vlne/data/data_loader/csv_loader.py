...
class CSVLoader(IDataLoader):
    ...
    files that `vlne` relies on.

    The `vlne` uses both slice level and prong level data. The slice level
    ...
    To store variable length arrays in the csv files, `vlne` serializes them
